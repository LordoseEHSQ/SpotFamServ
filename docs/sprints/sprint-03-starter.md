...
- **Offene, noch NICHT gemergte PRs aus dem Vorchat (CI-Status prüfen!):**
  - **#29** README-Versionen (Doku-Fix).
  - **#30** Planungs-Doku + Decisions D-015..D-017 (enthält u.a. diesen Starter).
  - **#31** `feat/reader-box-mapping` – Reader→Box **implementiert** (Schema+Code+Frontend+Tests+OpenAPI+Docs),
    aber **lokal NICHT voll getestet** (Host-PHP 8.3 < benötigt 8.4/8.5; nur `php -l`/`tsc -b`/Linter grün).
...
1. **Reader→Box (#31) abschließen:** CI grün, mergen, Pi-Migration + echtes Multi-Raum-E2E (Leser A→Box X, Leser B→Box Y; Leser ohne Mapping = Profil-Default). Plan: `plan-reader-box-mapping.md`.
...
- A (`explore`/Umsetzung): #31 CI-Fehler triagieren + fixen, Merge-Readiness.
...
Zuerst GitHub-Milestone „Sprint 3" + WorkPackage-Issues (Label `work-package`) anlegen (SSoT), PRs #29/#30/#31 prüfen/mergen,
dann `tasks/plan-sprint-03.md` (4-Lens + Cross-Module + Dry-Run) schreiben und auf User-Bestätigung warten. Erst danach Code.
